    ...
    request(url, function (error, response, html) {
      if (!error) {
        ...
        $('.eventbox').each(function(index, element) {
          ...
          var sideId1 = SportsBookGateway.getTeamIdFromId(data.find(".spread a").first().attr().id);
          var sideId2 = SportsBookGateway.getTeamIdFromId(data.find(".spread a").last().attr().id);
